...
A avaliação da plataforma evidenciou que os objetivos propostos foram alcançados, uma vez que o sistema apresentou melhorias significativas em relação ao modelo anterior baseado em planilhas. O entrevistado destacou que a navegação se tornou mais simples e intuitiva, especialmente em dispositivos móveis, confirmando o sucesso da proposta de ampliar a usabilidade. Além disso, a organização das informações foi otimizada por meio das telas específicas para cursos, professores e salas, respondendo diretamente à necessidade de tornar o acesso aos horários mais claro e rápido.

Outro ponto relevante foi a contribuição do sistema para reduzir dificuldades na gestão acadêmica, indicando que a plataforma não apenas substituiu a planilha, mas agregou valor ao fluxo de trabalho. A satisfação expressa pelo usuário reforça a adequação da solução às demandas reais do setor de ensino, demonstrando que a proposta se mostrou eficaz como ferramenta de apoio à gestão. Além disso, foram levantadas sugestões de melhorias, como a integração com outros sistemas institucionais e a inclusão de uma funcionalidade de validação de dados. Essas recomendações foram incorporadas na segunda versão, mostrando que a avaliação serviu como base para a evolução do projeto.

Por fim, a preocupação com a hospedagem institucional e a documentação técnica garante a sustentabilidade da solução, assegurando que a plataforma possa ser mantida e ampliada no futuro. Dessa forma, os resultados obtidos confirmam a efetividade da proposta e demonstram que o sistema cumpriu sua finalidade de modernizar o processo de disponibilização e consulta dos horários acadêmicos, superando limitações anteriores e estabelecendo bases sólidas para sua continuidade.